  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (!employeeId || !photoUrl) {
        return NextResponse.json(
          { error: "Employee ID and photo URL are required" },
          { status: 400 },
        );
      }
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!employeeUid || !employeeId) {
        return NextResponse.json(
          { error: "Employee UID and ID are required" },
          { status: 400 },
        );
      }
      ...
      if (fileType === "photo") {
        ...
        filename = `profileImage${employeeUid}.jpg`;
      } else {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!employeeUid) {
      return NextResponse.json(
        { error: "Employee UID is required" },
        { status: 400 },
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!employeeUid || !documentId) {
      return NextResponse.json(
        { error: "Employee UID and document ID are required" },
        { status: 400 },
      );
    }
