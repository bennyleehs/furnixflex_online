  ...
  try {
    ...
    let sql = `
      SELECT  
        ...
        branches2.name as branch, departments.name as department, roles1.name AS role,
        ...
        users1.status
      FROM users1
        LEFT JOIN branches2 ON branches2.id = users1.branch_id
        LEFT JOIN departments ON departments.id = users1.department_id
        LEFT JOIN roles1 ON roles1.id = users1.role_id
    `;
    ...
    if (id) {
      ...
      return NextResponse.json(id ? rows[0] : { listEmployee: rows }, { status: 200 });
    } catch (error) {
