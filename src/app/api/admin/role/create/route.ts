...
export async function PUT(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ success: false, error: 'Role ID is required for update' }, { status: 400 });
    }
    ...
    if (result.affectedRows === 0) {
      return NextResponse.json({ success: false, error: 'Role not found or no changes made' }, { status: 404 });
    }
