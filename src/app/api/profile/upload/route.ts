...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (!usersId || !photoUrl) {
        return NextResponse.json(
          { error: "User ID and photo URL are required" },
          { status: 400 },
        );
      }
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!usersUid || !usersId) {
        return NextResponse.json(
          { error: "User UID and ID are required" },
          { status: 400 },
        );
      }
      ...
      const fileUrl = `/api/files/admin/employee/${usersUid}/upload/${filename}?v=${Date.now()}`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const profilePhotoPath = path.join(
      uploadDir,
      `profileImage${usersUid}.jpg`,
    );
    const profilePhotoUrl = existsSync(profilePhotoPath)
      ? `/api/files/admin/employee/${usersUid}/upload/profileImage${usersUid}.jpg`
      : null;
    ...
    const documents = files
      ...
      .map((file) => {
        ...
        return {
          ...
          url: `/api/files/admin/employee/${usersUid}/upload/${file}`,
        };
      });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!usersUid || !documentId) {
      return NextResponse.json(
        { error: "User UID and document ID are required" },
        { status: 400 },
      );
    }
