...
export async function POST(req: NextRequest) {
  try {
    ...
    const itemInfo = allPermissionInfo.find(item => String(item.id) === String(permissionValue));
    if (itemInfo && DEFAULT_ACCESS_SECTIONS.includes(itemInfo.label)) {
      console.log(`${permissionValue} belongs to default access section ${itemInfo.label}`);
    ...
    const hasPermission = permissions.some(permission => {
      ...
      const match = permStr === reqPermStr || isParentPermission(permStr, reqPermStr);
      if (match) {
        console.log(`Permission match found: ${permission} grants access to ${permissionValue}`);
      }
      return match;
    });
