...
doc.text(new Date().toLocaleDateString(), valueC3, sectionTop + 15);
  ...
  const salesRepWidth = data.sales_representative ? doc.getTextWidth(data.sales_representative) + 1 : 0;
  if (data.sales_uid) {
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(paymentSummaryTitle, pageWidth / 2, paymentSummaryY, { align: 'center' });
  doc.line((pageWidth / 2) - (paymentSummaryTitleWidth/2), paymentSummaryY+2, pageWidth / 2 + paymentSummaryTitleWidth/2, paymentSummaryY+2);
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('Total Amount', margin + boxWidth / 2, paymentSummaryY + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCurrency(data.quotation_total), margin + boxWidth / 2, paymentSummaryY + 20, { align: 'center' });
  
  // Paid amount box
  doc.setDrawColor(220, 220, 220);
  doc.setFillColor(240, 255, 240);
  doc.rect(margin + boxWidth + 5, paymentSummaryY + 5, boxWidth, boxHeight, 'F');
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('Amount Paid', margin + boxWidth + 5 + boxWidth / 2, paymentSummaryY + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(0, 128, 0);
  doc.text(formatCurrency(data.total_paid), margin + boxWidth + 5 + boxWidth / 2, paymentSummaryY + 20, { align: 'center' });
  doc.setTextColor(0, 0, 0);
  
  // Balance box
  doc.setDrawColor(220, 220, 220);
  doc.setFillColor(255, 245, 240);
  doc.rect(margin + (boxWidth * 2) + 10, paymentSummaryY + 5, boxWidth, boxHeight, 'F');
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('Remaining Balance', margin + (boxWidth * 2) + 10 + boxWidth / 2, paymentSummaryY + 12, { align: 'center' });
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(220, 53, 69);
  doc.text(formatCurrency(data.balance), margin + (boxWidth * 2) + 10 + boxWidth / 2, paymentSummaryY + 20, { align: 'center' });
  doc.setTextColor(0, 0, 0);
  ...
  const progressPercentage = typeof data.payment_progress === 'number' 
    ? data.payment_progress 
    : parseFloat(data.payment_progress) || 0;
  
  doc.text(`Payment Progress: ${progressPercentage.toFixed(0)}%`, margin, progressBarY - 2);
  
  // Background (empty) progress bar
  doc.setDrawColor(220, 220, 220);
  doc.setFillColor(220, 220, 220);
  doc.roundedRect(margin, progressBarY, progressBarWidth, progressBarHeight, 1, 1, 'F');
  
  // Filled progress bar
  const filledWidth = Math.min(progressBarWidth * (progressPercentage / 100), progressBarWidth);
  doc.setDrawColor(40, 167, 69);
  doc.setFillColor(40, 167, 69);
  if (filledWidth > 0) {
    doc.roundedRect(margin, progressBarY, filledWidth, progressBarHeight, 1, 1, 'F');
  }
  ...
  const tableBody = data.payments.map((payment: any) => [
    ...
    payment.received && payment.received_date ? formatDate(payment.received_date) : '-' // Format the received date or show dash if not available
  ]);
    ...
    didDrawCell: function(data) {
      ...
      if (data.section === 'head') {
        if (data.column.index === 3 || data.column.index === 4 || 
            data.column.index === 5 || data.column.index === 6) { // Add column 6 to headers to center
      ...
      // Status cell styling
      if (data.section === 'body' && data.column.index === 5 && data.cell.raw === 'RECEIVED') {
        doc.setTextColor(40, 167, 69);
      } else if (data.section === 'body' && data.column.index === 5 && data.cell.raw === 'PENDING') {
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.text('SYSTEM-GENERATED STATEMENT', pageWidth / 2, footerY - 8, { align: 'center' });

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.text('This is an electronically generated payment statement. No signature is required.', pageWidth / 2, footerY - 4, { align: 'center' });

  // Standard footer info
  doc.setFontSize(7);
  doc.text('Generated on: ' + new Date().toLocaleDateString(), margin, footerY);
  doc.text('Page 1 of 1', pageWidth - margin, footerY, { align: 'right' });
  doc.text('Powered by CLASSYPRO System', pageWidth / 2, footerY, { align: 'center' });
