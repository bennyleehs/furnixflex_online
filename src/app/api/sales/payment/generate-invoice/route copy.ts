...
export async function POST(request: Request) {
  try {
    ...
    const uploadsDir = path.join(process.cwd(), 'public', 'sales', data.quotation.task_id, 'Invoice');
    ...
    let fileIndex = 0;
    let fileName = `I-${safeFilename}.pdf`;
    let filePath = path.join(uploadsDir, fileName);
    
    while (fs.existsSync(filePath)) {
      fileIndex++;
      fileName = `I-${safeFilename}-${fileIndex.toString()}.pdf`;
