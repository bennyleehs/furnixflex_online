...
//     const id = searchParams.get("id") || null;
// // NEW: Get the user's UID for filtering
...
//       //   params.push(status);
//       // }
//       // NEW: Add userUid filter as the first condition
//   if (userUid) {
//     whereClause = "WHERE e.uid = ?";
//     params.push(userUid);
//   }

//   // Apply status filter. If userUid is already set, use 'AND'
//   if (status) {
...
// // NEW: Create a new parameter array for the status count query, which uses the same params as the main query
...
//         const searchValue = `%${search}%`;
//         // Push the search parameter for each column
//         for (let i = 0; i < 15; i++) {
//           queryParams.push(searchValue);
//         }
//         whereClause += ")"; // Close the parenthesis for the search group
//       }
//     }
...
//     // Status counts query needs to be updated to include the userUid filter
//     const statusCountSql = `
//       SELECT c.status, COUNT(*) as count
//       FROM customers c
//       LEFT JOIN users e ON c.sales_id = e.id
...
//     // Use an array to build the WHERE clause conditions
...
//     // Define privileged roles that can see all tasks
...
//       queryParams.push(id);
//     } else {
//       // ONLY add the user UID filter if the user is NOT a privileged role
//       if (!canSeeAllTasks) {
//         if (userUid) { // Ensure userUid is not null before adding the condition
//           whereConditions.push("e.uid = ?");
//           queryParams.push(userUid);
//         } else {
//           // If no UID is provided for a non-privileged user, return no results.
//           return new Response(
//             JSON.stringify({
//               listTask: [],
//               totalCount: 0,
    ...
    const dataSql = `
      ...
      ORDER BY 
        CASE c.status
          ...
          ELSE 8 -- This will put 'Job Done' and any other unexpected status at the bottom
        END,
